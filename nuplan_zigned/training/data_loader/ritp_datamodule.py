...
# from nuplan_zigned.training.preprocessing.ritp_feature_collate import FeatureCollate
...
def set_worker_sharing_strategy(worker_id: int) -> None:
    """
    set this to fix "RuntimeError: received 0 items of ancdata" and "RuntimeError: unable to open shared memory object".
    However, this seems to slow down training process.
    """
    # print('default:', torch.multiprocessing.get_sharing_strategy())
    torch.multiprocessing.set_sharing_strategy(sharing_strategy)
    # print('after:', torch.multiprocessing.get_sharing_strategy())
...
def create_dataset(
    ...
    :param num_rl_workers: Number of RL workers.
    ...
    for scenario in selected_scenarios:
        scenario.is_last_scenario = False

    logger.info(f"Number of samples in {dataset_name} set: {len(selected_scenarios)}")

    repeated_scenarios = []
    split_scenarios = [selected_scenarios[i:i+num_rl_workers] for i in range(0, len(selected_scenarios), num_rl_workers)]

    for scenario in split_scenarios[-1]:
        scenario.is_last_scenario = True
    for split in split_scenarios:
        if len(split) < num_rl_workers:
            # split += selected_scenarios[0:num_rl_workers - num_keep % num_rl_workers]
            split += selected_scenarios[-1:] * (num_rl_workers - num_keep % num_rl_workers)
        num_of_iterations = [scenario.get_number_of_iterations() for scenario in split]
        num_of_iterations = min(num_of_iterations)

        repeated_scenarios += split * num_of_iterations

    logger.info("Each scenario has been repeated for num_of_iterations times")
    logger.info(f"Number of samples in {dataset_name} set: {len(repeated_scenarios)}")

    return ScenarioDataset(
        scenarios=repeated_scenarios,
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        # RL worker
        self._num_rl_workers = num_rl_workers
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == 'fit':
            ...
            tr_tokens = [s.token for s in train_samples]
            val_tokens = [s.token for s in val_samples]
            print(f'first 10 training scenarios are: {tr_tokens[:10]}')
            print(f'first 10 validation scenarios are: {val_tokens[:10]}')
