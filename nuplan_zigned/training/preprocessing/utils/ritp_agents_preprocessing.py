...
def build_generic_ego_features_from_tensor(ego_trajectory: torch.Tensor) -> torch.Tensor:
    """
    ...
    global_ego_trajectory_output: torch.Tensor = torch.empty(
        ego_trajectory.size(), dtype=torch.float32, device=ego_trajectory.device
    )
    global_ego_trajectory_output[:, EgoInternalIndex.x()] = global_ego_poses[:, 0].float()
    global_ego_trajectory_output[:, EgoInternalIndex.y()] = global_ego_poses[:, 1].float()
    global_ego_trajectory_output[:, EgoInternalIndex.heading()] = global_ego_poses[:, 2].float()
    global_ego_trajectory_output[:, EgoInternalIndex.vx()] = global_ego_velocities[:, 0].float()
    global_ego_trajectory_output[:, EgoInternalIndex.vy()] = global_ego_velocities[:, 1].float()
    global_ego_trajectory_output[:, EgoInternalIndex.ax()] = global_ego_accelerations[:, 0].float()
    global_ego_trajectory_output[:, EgoInternalIndex.ay()] = global_ego_accelerations[:, 1].float()

    return global_ego_trajectory_output


def filter_agents_tensor(agents: List[torch.Tensor],
                         tracked_token_ids: Dict[str, int],
                         reverse: bool = False) -> Tuple[List[torch.Tensor], Dict[str, int]]:
    """
        ...
        for j in range(agents[i].shape[0]):
            if target_tensor.shape[0] > 0:
                ...
                if is_in_target_frame:
                    ...
                    output_tracked_token_ids[list(tracked_token_ids.keys())[int(agent_id)]] = int(agent_id)
    ...
    return agents, output_tracked_token_ids


def filter_agents_by_distance(ego_history: torch.Tensor,
                              list_agent_states: List[torch.Tensor],
                              tracked_token_ids: Dict[str, int],
                              a2ego_radius: float,
                              max_num_agent: Optional[int]) -> Tuple[List[torch.Tensor], Dict[str, int]]:
    """
    Filter agents out of radius with respect to the ego vehicle at each timestep.
    Consider a max number of each type of agents.
    """
    output_list = []
    output_tracked_token_ids = {}
    anchor_states = ego_history[:, [GenericAgentFeatureIndex.x(), GenericAgentFeatureIndex.y()]]
    distances = []

    for i_agent in range(len(list_agent_states)):
        agent_states = list_agent_states[i_agent]
        states = agent_states[:, [AgentInternalIndex.x(), AgentInternalIndex.y()]]
        timesteps = agent_states[:, -1].long()
        distance = torch.norm(anchor_states[timesteps] - states, dim=1).min()
        distances.append(distance)

    if len(distances) > 0:
        sort = torch.stack(distances, dim=0).argsort(descending=False)
        for i_agent in range(sort.size(0)):
            if distances[sort[i_agent]] < a2ego_radius:
                output_list.append(list_agent_states[sort[i_agent]])
                key = list(tracked_token_ids.keys())[sort[i_agent]]
                value = list(tracked_token_ids.values())[sort[i_agent]]
                output_tracked_token_ids[key] = value
            if max_num_agent is not None:
                if len(output_list) >= max_num_agent:
                    break

    return output_list, output_tracked_token_ids
...
def compute_yaw_rate_from_state_tensors(
    ...
    agent_frame_yaws: Dict[float, Dict[float, torch.Tensor]] = {}
    for i_time in range(len(agent_states)):
        for i_agent in range(len(agent_states[i_time])):
            track_token_float = float(agent_states[i_time][i_agent][AgentInternalIndex.track_token()])
            if track_token_float not in agent_frame_yaws.keys():
                agent_frame_yaws[track_token_float] = {}
            agent_frame_yaws[track_token_float][i_time] = agent_states[i_time][i_agent][AgentInternalIndex.heading()]

    # Remove [-pi, pi] yaw bounds to make the signal smooth
    agent_frame_yaws_smoothed: Dict[float, Dict[int, torch.Tensor]] = {}
    for agent, agent_yaw in agent_frame_yaws.items():
        timestep = torch.tensor(list(agent_yaw.keys()))
        yaw = torch.tensor(list(agent_yaw.values()))
        if timestep.size(0) < timestep[-1] - timestep[0] + 1:
            fun = interp1d(time_stamps_s[timestep], yaw)
            timestep_ = torch.arange(timestep[0], timestep[-1] + 1)
            yaw_ = torch.tensor(fun(time_stamps_s[timestep_]), dtype=torch.float32)
        else:
            timestep_ = timestep
            yaw_ = yaw
        yaw_ = unwrap(yaw_.double())
        agent_frame_yaws_smoothed[agent] = {}
        for i in range(timestep_.size(0)):
            if timestep_[i] in list(agent_frame_yaws[agent].keys()):
                agent_frame_yaws_smoothed[agent][timestep_[i]] = yaw_[i]

    # calculate yaw rate
    agent_frame_yaw_rates: Dict[float, torch.Tensor] = {}
    agent_timesteps: Dict[float, torch.Tensor] = {}
    for agent, yaw in agent_frame_yaws_smoothed.items():
        timestep = torch.tensor(list(agent_frame_yaws_smoothed[agent].keys()))
        yaw_tensor = torch.tensor((list(yaw.values())))
        if len(yaw.values()) > 2:
            agent_frame_yaw_rates[agent] = approximate_derivatives_tensor(
                torch.unsqueeze(yaw_tensor, dim=0),
                time_stamps_s[timestep],
                window_length=3
            ).squeeze()
        else:
            agent_frame_yaw_rates[agent] = yaw_tensor.new_zeros(size=yaw_tensor.size())
        agent_timesteps[agent] = timestep

    # Convert back to frame x agent
    frame_agent_yaw_rates: Dict[int, Dict[float, torch.Tensor]] = {}
    for agent, yaw_rate in agent_frame_yaw_rates.items():
        timesteps = agent_timesteps[agent]
        for i_timestep in range(timesteps.size(0)):
            if int(timesteps[i_timestep]) not in frame_agent_yaw_rates.keys():
                frame_agent_yaw_rates[int(timesteps[i_timestep])] = {}
            frame_agent_yaw_rates[int(timesteps[i_timestep])][agent] = yaw_rate[i_timestep]
    for i_timestep in range(len(time_stamps_s)):
        if i_timestep not in frame_agent_yaw_rates.keys():
            frame_agent_yaw_rates[i_timestep] = {}

    return frame_agent_yaw_rates


def sampled_ego_states_to_tensor(ego_states: List[EgoState]) -> torch.Tensor:
    """
    ...
    output = torch.zeros((len(ego_states), GenericAgentFeatureIndex.dim()), dtype=torch.float32)
    for i in range(0, len(ego_states), 1):
        output[i, GenericAgentFeatureIndex.x()] = ego_states[i].agent.center.x
        output[i, GenericAgentFeatureIndex.y()] = ego_states[i].agent.center.y
        output[i, GenericAgentFeatureIndex.heading()] = ego_states[i].agent.center.heading
        output[i, GenericAgentFeatureIndex.vx()] = (ego_states[i].agent.velocity.x * np.cos(ego_states[i].agent.center.heading)
                                                    - ego_states[i].agent.velocity.y * np.sin(ego_states[i].agent.center.heading))
        output[i, GenericAgentFeatureIndex.vy()] = (ego_states[i].agent.velocity.x * np.sin(ego_states[i].agent.center.heading)
                                                    + ego_states[i].agent.velocity.y * np.cos(ego_states[i].agent.center.heading))
        output[i, GenericAgentFeatureIndex.width()] = ego_states[i].agent.box.width
        output[i, GenericAgentFeatureIndex.length()] = ego_states[i].agent.box.length
        output[i, GenericAgentFeatureIndex.timestep()] = i
        output[i, GenericAgentFeatureIndex.track_token()] = -1

    return output


def sampled_timestamps_to_tensor(time_stamps: List[TimePoint]) -> torch.Tensor:
    """
    Converts a list of N past timestamps into a 1-d tensor of shape [N]. The field is the timestamp in uS.
    :param time_stamps: The time stamps to convert.
    :return: The converted tensor.
    """
    flat = [t.time_us for t in time_stamps]
...
def pack_agents_tensor(agents_tensors: List[torch.Tensor]) -> List[torch.Tensor]:
    """
    Pack the agents states into the final output feature tensor.
    :param agents_tensors: The agent states for each timestamp.
        Each tensor is of shape <num_agents, len(AgentInternalIndex) + 1> and conforms to the AgentInternalIndex schema.
    :return: The final feature, a list of tensor of shape [num_agents, len(AgentsFeatureIndex)] conforming to the AgentFeatureIndex Schema
    """
    output_agents_tensors: List[torch.Tensor] = []

    for i_agent in range(len(agents_tensors)):
        agent_tensor = torch.zeros(
            (agents_tensors[i_agent].shape[0], GenericAgentFeatureIndex.dim())
        )
        agent_tensor[:, GenericAgentFeatureIndex.x()] = agents_tensors[i_agent][:, AgentInternalIndex.x()]
        agent_tensor[:, GenericAgentFeatureIndex.y()] = agents_tensors[i_agent][:, AgentInternalIndex.y()]
        agent_tensor[:, GenericAgentFeatureIndex.heading()] = agents_tensors[i_agent][:, AgentInternalIndex.heading()]
        agent_tensor[:, GenericAgentFeatureIndex.vx()] = agents_tensors[i_agent][:, AgentInternalIndex.vx()]
        agent_tensor[:, GenericAgentFeatureIndex.vy()] = agents_tensors[i_agent][:, AgentInternalIndex.vy()]
        agent_tensor[:, GenericAgentFeatureIndex.width()] = agents_tensors[i_agent][:, AgentInternalIndex.width()]
        agent_tensor[:, GenericAgentFeatureIndex.length()] = agents_tensors[i_agent][:, AgentInternalIndex.length()]
        agent_tensor[:, GenericAgentFeatureIndex.timestep()] = agents_tensors[i_agent][:, -1]
        agent_tensor[:, GenericAgentFeatureIndex.track_token()] = agents_tensors[i_agent][:, AgentInternalIndex.track_token()]
        output_agents_tensors.append(agent_tensor)

    return output_agents_tensors


def get_agent_states(agent_history: List[torch.Tensor]) -> List[torch.Tensor]:
    """
    Reshape agent_history to agent_states. Note that "history" = "past" + "present".
    :param agent_history: agent trajectories [num_frames, num_agents, AgentInternalIndex.dim()], corresponding to the AgentInternalIndex schema.
    :return: agent trajectories [num_agents, num_frames, AgentInternalIndex.dim() + 1], corresponding to the AgentInternalIndex schema.
    """
    num_agents = agent_history[-1].shape[0]
    num_frames = len(agent_history)
    track_tokens = agent_history[-1][:, 0]
    list_agent_state = []

    # add a dimension of timestep
    agent_history_ = []
    for i in range(num_frames):
        agent_history_.append(torch.cat([agent_history[i], torch.ones((agent_history[i].shape[0], 1)) * i], dim=1))

    agent_history_ = torch.cat(agent_history_, dim=0)

    for i in range(num_agents):
        agent_states = agent_history_[agent_history_[:, 0] == track_tokens[i]]
        list_agent_state.append(agent_states)

    return list_agent_state
