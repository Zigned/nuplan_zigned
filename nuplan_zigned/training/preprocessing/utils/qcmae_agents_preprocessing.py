...
def filter_agents_by_distance(ego_history: torch.Tensor,
    ...
    if len(distances) > 0:
        ...
        for i_agent in range(sort.size(0)):
            if distances[sort[i_agent]] < a2a_radius:
...
def sampled_past_ego_states_to_tensor(past_ego_states: List[EgoState]) -> torch.Tensor:
    ...
    output = torch.zeros((len(past_ego_states), GenericAgentFeatureIndex.dim()), dtype=torch.float32)
    for i in range(0, len(past_ego_states), 1):
        output[i, GenericAgentFeatureIndex.x()] = past_ego_states[i].agent.center.x
        output[i, GenericAgentFeatureIndex.y()] = past_ego_states[i].agent.center.y
        output[i, GenericAgentFeatureIndex.heading()] = past_ego_states[i].agent.center.heading
        output[i, GenericAgentFeatureIndex.vx()] = (past_ego_states[i].agent.velocity.x * np.cos(past_ego_states[i].agent.center.heading)
                                                    - past_ego_states[i].agent.velocity.y * np.sin(past_ego_states[i].agent.center.heading))
        output[i, GenericAgentFeatureIndex.vy()] = (past_ego_states[i].agent.velocity.x * np.sin(past_ego_states[i].agent.center.heading)
                                                    + past_ego_states[i].agent.velocity.y * np.cos(past_ego_states[i].agent.center.heading))
        output[i, GenericAgentFeatureIndex.width()] = past_ego_states[i].agent.box.width
        output[i, GenericAgentFeatureIndex.length()] = past_ego_states[i].agent.box.length
