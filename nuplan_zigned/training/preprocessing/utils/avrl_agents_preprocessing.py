...
def pack_agents_tensor(agents_tensors: List[torch.Tensor],
                       frame_agent_yaw_rates: Dict[int, Dict[float, torch.Tensor]])\
        -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    """
    Combines the agents states and the computed yaw rates into the final output feature tensor.
    ...
    :param frame_agent_yaw_rates: The computed yaw rates.
    ...
    """
    if len(frame_agent_yaw_rates.keys()) != len(agents_tensors):
        raise ValueError(f"Unexpected yaw_rates tensor shape: {len(frame_agent_yaw_rates.keys())}")

    output_agents_tensors: List[torch.Tensor] = []
    output_agents_ids: List[torch.Tensor] = []

    for i_timestep in range(len(agents_tensors)):
        _validate_agent_internal_shape(agents_tensors[i_timestep])
        agents_tensor = torch.zeros(
            (agents_tensors[i_timestep].shape[0], AgentFeatureIndex.dim())
        )
        agents_id = torch.zeros(
            (agents_tensors[i_timestep].shape[0], 1)
        )
        agent_yaw_rate = list(frame_agent_yaw_rates[i_timestep].values())
        if len(agent_yaw_rate) > 0:
            for i_agent in range(agents_tensors[i_timestep].shape[0]):
                agents_tensor[i_agent, AgentFeatureIndex.x()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.x()]
                agents_tensor[i_agent, AgentFeatureIndex.y()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.y()]
                agents_tensor[i_agent, AgentFeatureIndex.heading()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.heading()]
                agents_tensor[i_agent, AgentFeatureIndex.vx()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.vx()]
                agents_tensor[i_agent, AgentFeatureIndex.vy()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.vy()]
                agents_tensor[i_agent, AgentFeatureIndex.yaw_rate()] = agent_yaw_rate[i_agent]
                agents_tensor[i_agent, AgentFeatureIndex.width()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.width()]
                agents_tensor[i_agent, AgentFeatureIndex.length()] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.length()]
                agents_id[i_agent, 0] = agents_tensors[i_timestep][i_agent, AgentInternalIndex.track_token()]
        output_agents_tensors.append(agents_tensor)
        output_agents_ids.append(agents_id)

    return output_agents_tensors, output_agents_ids
