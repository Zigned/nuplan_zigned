...
class Trajectory(AbstractModelFeature):
    ...
    @classmethod
    def transform_to_local_frame_given_anchor_state(cls, data: FeatureDataType, anchor_state: Agent) -> Trajectory:
        """
        Transform from global frame to local frame.
        """
        origin = anchor_state.center.array
        theta = np.array([anchor_state.center.heading])
        ...
        rot_mat[1, 0] = sin
        rot_mat[1, 1] = cos
        trajectory = np.zeros_like(data)
        trajectory[:, :2] = (data[:, :2] - origin) @ rot_mat
        trajectory[:, 2] = wrap_angle(data[:, 2] - theta)

        return cls(data=trajectory)