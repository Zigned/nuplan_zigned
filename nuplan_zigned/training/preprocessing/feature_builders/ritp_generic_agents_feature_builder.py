...
from nuplan_zigned.training.preprocessing.utils.ritp_agents_preprocessing import (
    get_agent_states,
    pack_agents_tensor,
    sampled_ego_states_to_tensor,
    sampled_tracked_objects_to_tensor_list,
    sampled_timestamps_to_tensor,
    filter_agents_tensor,
    filter_agents_by_distance,
)
from nuplan_zigned.training.preprocessing.features.ritp_generic_agents import (
    GenericAgents,
...
class GenericAgentsFeatureBuilder(ScriptableFeatureBuilder):
    ...
    def __init__(self,
        ...
        self.a2ego_radius = a2ego_radius
    ...
    Dict[str, Union[Dict[str, int], Any]]]:
        ...
        future_ego_state = scenario.get_ego_future_trajectory(
            iteration=0, num_samples=self.number_of_interations, time_horizon=self.scenario_time_horizon
        )
        sampled_ego_states = sampled_past_ego_states + list(future_ego_state)
        time_stamps = list(
            scenario.get_past_timestamps(
                iteration=0, num_samples=self.num_past_steps, time_horizon=self.past_time_horizon
            )
        ) + [scenario.start_time] + future_timestamps

        # Retrieve past/present/future agent boxes
        ...
        sampled_observations = sampled_past_observations + list(future_tracked_obsjects)
    ...
    def _pack_to_feature_tensor_dict(
        ...
        :param ego_states: The past+present+future states of the ego vehicle.
        :param time_stamps: The past+present+future time stamps of the input data.
        :param tracked_objects: The past+present+future tracked objects.
        :return: The packed tensors.
        """
        list_tensor_data: Dict[str, List[torch.Tensor]] = {}
        ego_states_tensor = sampled_ego_states_to_tensor(ego_states)
        time_stamps_tensor = sampled_timestamps_to_tensor(time_stamps)
        track_token_ids = {}

        for feature_name in self.agent_features:
            tracked_objects_tensor_list, track_token_ids[feature_name] = sampled_tracked_objects_to_tensor_list(
                tracked_objects, TrackedObjectType[feature_name]
            )
            list_tensor_data[f"tracked_objects.{feature_name}"] = tracked_objects_tensor_list

        return (
            {
                "ego_states": ego_states_tensor,
    ...
    Dict[str, Union[Dict[str, int], Any]]]:
        ...
        ego_history: torch.Tensor = tensor_data["ego_states"]
        ...
        for feature_name in self.agent_features:
            ...
            if f"tracked_objects.{feature_name}" in list_tensor_data:
                agents: List[torch.Tensor] = list_tensor_data[f"tracked_objects.{feature_name}"]
    ...
    def get_agent_data(self,
        ...
        # initialization
        valid_mask = np.zeros((num_agents, self.number_of_interations, self.num_steps), dtype=bool)
        current_valid_mask = np.zeros((num_agents, self.number_of_interations), dtype=bool)
        predict_mask = np.zeros((num_agents, self.number_of_interations, self.num_steps), dtype=bool)
        position = np.zeros((num_agents, self.number_of_interations, self.num_historical_steps, 2), dtype=float)
        heading = np.zeros((num_agents, self.number_of_interations, self.num_historical_steps), dtype=float)
        velocity = np.zeros((num_agents, self.number_of_interations, self.num_historical_steps, 2), dtype=float)
        length = np.zeros((num_agents,), dtype=float)
        width = np.zeros((num_agents,), dtype=float)
        ...
        for agent_id, agent_data in zip(agent_ids, agents_data):
            ...
            length[agent_idx] = agent_data[0, GenericAgentFeatureIndex.length()]
            width[agent_idx] = agent_data[0, GenericAgentFeatureIndex.width()]
            for iteration in range(self.number_of_interations):
                agent_data_at_iter = agent_data[(agent_steps >= iteration) &
                                                (agent_steps < iteration + self.num_historical_steps)]
                agent_steps_at_iter = agent_steps[(agent_steps >= iteration) &
                                                  (agent_steps < iteration + self.num_historical_steps)]
                valid_mask[
                    agent_idx,
                    iteration,
                    agent_steps_at_iter - iteration
                ] = True
                current_valid_mask[agent_idx, iteration] = valid_mask[agent_idx, iteration, self.num_historical_steps - 1]
                predict_mask[agent_idx, iteration] = True  # default to predict all future steps

                # a time step t is valid only when both t and t-1 are valid
                valid_mask[agent_idx, iteration, 1: self.num_historical_steps] = (
                        valid_mask[agent_idx, iteration, :self.num_historical_steps - 1] &
                        valid_mask[agent_idx, iteration, 1: self.num_historical_steps])
                valid_mask[agent_idx, iteration, 0] = False

                predict_mask[agent_idx, iteration, :self.num_historical_steps] = False
                if not current_valid_mask[agent_idx, iteration]:
                    predict_mask[agent_idx, iteration, self.num_historical_steps:] = False

                position[agent_idx, iteration, agent_steps_at_iter - iteration, :2] = np.stack([agent_data_at_iter[:, GenericAgentFeatureIndex.x()],
                                                                                    agent_data_at_iter[:, GenericAgentFeatureIndex.y()]],
                                                                                   axis=-1)
                heading[agent_idx, iteration, agent_steps_at_iter - iteration] = agent_data_at_iter[:, GenericAgentFeatureIndex.heading()]
                velocity[agent_idx, iteration, agent_steps_at_iter - iteration, :2] = np.stack([agent_data_at_iter[:, GenericAgentFeatureIndex.vx()],
                                                                                    agent_data_at_iter[:, GenericAgentFeatureIndex.vy()]],
                                                                                   axis=-1)
