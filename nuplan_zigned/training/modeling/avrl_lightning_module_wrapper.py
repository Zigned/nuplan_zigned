...
class LightningModuleWrapper(pl.LightningModule):
    """
    Lightning module that wraps the training/validation/testing procedure and handles the objective/training_metric computation.
    """
    ...
    def __init__(
        ...
        """
        super().__init__()
        # self.save_hyperparameters(ignore=["model", "objectives"])  # ignoring "objectives" cause bug in simulation
    ...
    def _step(self, batch: Tuple[FeaturesType, TargetsType, ScenarioListType], prefix: str) -> torch.Tensor:
        ...
        """
        if prefix == 'train' and self.model.validate:
            return None
        ...
        if self.model.validate:
            objectives = {objective.name(): torch.Tensor([0.]) for objective in self.objectives}
        else:
    ...
    def _log_step(
        ...
        for key, value in metrics.items():
            self.log(f'metrics/{prefix}_{key}', value, prog_bar=True, on_step=False, on_epoch=True)
