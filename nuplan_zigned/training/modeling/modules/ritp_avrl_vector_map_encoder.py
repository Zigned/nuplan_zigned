...
class RewardMapEncoder(nn.Module):
    ...
    def forward(self, features: FeaturesType) -> Dict[str, List[Union[torch.Tensor, Any]]]:
        ...
        for sample_idx in range(features['vector_set_map'].batch_size):
            ...
            pos_pt = map_data['map_point']['position'][sample_idx][:, :self.input_dim].contiguous()
            orient_pt = map_data['map_point']['orientation'][sample_idx].contiguous()
            ...
            x_pt = map_data['map_point']['magnitude'][sample_idx].unsqueeze(-1)
            x_pl = None
            x_pt_categorical_embs = [self.type_pt_emb(map_data['map_point']['type'][sample_idx].long()),
                                     self.side_pt_emb(map_data['map_point']['side'][sample_idx].long()),
                                     self.tl_pt_emb(map_data['map_point']['tl_statuses'][sample_idx].long())]
            x_pl_categorical_embs = [self.type_pl_emb(map_data['map_polygon']['type'][sample_idx].long()),
                                     self.int_pl_emb(map_data['map_polygon']['is_intersection'][sample_idx].long())]
            ...
            edge_index_pt2pl = map_data['map_point', 'to', 'map_polygon']['edge_index'][sample_idx].long()
            ...
            edge_index_pl2pl = map_data['map_polygon', 'to', 'map_polygon']['edge_index'][sample_idx].long()
            type_pl2pl = map_data['map_polygon', 'to', 'map_polygon']['type'][sample_idx]
            ...
            r_pl2pl = self.r_pl2pl_emb(continuous_inputs=r_pl2pl,
                                       categorical_embs=[self.type_pl2pl_emb(type_pl2pl.long())])
            ...
            for i in range(self.num_layers):
                ...
                # self.pt2pl_layers[i].bools['store_dropout_mask'] = store_dropout_mask
                # self.pt2pl_layers[i].bools['use_frozen_dropout_mask'] = use_frozen_dropout_mask
                # self.pl2pl_layers[i].bools['store_dropout_mask'] = store_dropout_mask
                # self.pl2pl_layers[i].bools['use_frozen_dropout_mask'] = use_frozen_dropout_mask
