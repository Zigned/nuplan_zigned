...
class AttentionLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        self.to_q = nn.Sequential(
            MyDropout(p=dropout, bools=self.bools),
            nn.Linear(hidden_dim, head_dim * num_heads),
        )
        self.to_k = nn.Sequential(
            MyDropout(p=dropout, bools=self.bools),
            nn.Linear(hidden_dim, head_dim * num_heads, bias=False),
        )
        ...
        if has_pos_emb:
            self.to_k_r = nn.Sequential(
                MyDropout(p=dropout, bools=self.bools),
                nn.Linear(hidden_dim, head_dim * num_heads, bias=False),
            )
            self.to_v_r = nn.Sequential(
                MyDropout(p=dropout, bools=self.bools),
                nn.Linear(hidden_dim, head_dim * num_heads),
            )
        if gated_attention:
            if gate_has_dropout:
                ...
                self.to_g = nn.Sequential(
                    MyDropout(p=dropout, bools=self.bools),
                    nn.Linear(head_dim * num_heads + hidden_dim, head_dim * num_heads),
                )
            else:
        ...
        self.to_out = nn.Sequential(
            MyDropout(p=dropout, bools=self.bools),
            nn.Linear(head_dim * num_heads, hidden_dim),
        )
    ...
    def _attn_block(self,
        ...
        agg = self.propagate(edge_index=edge_index, x_dst=x_dst, q=q, k=k, v=v, r=r)  # in -> message -> update -> out
