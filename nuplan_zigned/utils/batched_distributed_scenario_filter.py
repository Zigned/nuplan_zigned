...
class DistributedScenarioFilter:
    ...
    def get_scenarios(self) -> List[AbstractScenario]:
        ...
        if 'scenario_filter_num_batches' in self._cfg.keys():
            assert 'scenario_filter_sample_idx' in self._cfg.keys()
            sample_idx = self._cfg.scenario_filter_sample_idx
            batch_size = len(scenarios) // self._cfg.scenario_filter_num_batches
            scenarios = scenarios[batch_size * sample_idx:batch_size * (sample_idx + 1)]
        return scenarios
