...
class TrainLoop:
    ...
    def __init__(self, trainer, multiple_trainloader_mode: str, replay_buffer: ReplayBuffer, env: Env):
    ...
    def run_training_epoch(self):
        ...
        # variables for RL
        num_rl_workers = self.trainer.lightning_module.model.num_rl_workers
        previous_scenario_tokens = [None] * num_rl_workers
        previous_rl_step = None
        self.env.iteration = [0] * num_rl_workers  # timestep
        skip = True
        ...
        for batch_idx, (batch, is_last_batch) in train_dataloader:
            ...
            if batch_idx == 0:
                self.env.log(f'batch_idx: {batch_idx}-scenarios_tokens: {scenario_tokens}')
            if len(set(scenario_tokens)) < len(scenario_tokens):
                print(f'scenario_tokens:{scenario_tokens}')
            # is_last_scenario = self.trainer.datamodule._train_set._scenarios[batch_idx * num_rl_workers].is_last_scenario

            # resume from latest step
            if isinstance(self.trainer.lightning_module.model.latest_step, str):
                if skip:
                    if scenario_tokens[0] != self.trainer.lightning_module.model.latest_step:
                        if batch_idx % 150 == 0:
                            print(f'skip: batch_idx {batch_idx}, scenario {scenario_tokens[0]} is not target scenario {self.trainer.lightning_module.model.latest_step}')
                        continue
                    else:
                        skip = False
            else:
                if self.trainer.lightning_module.model.latest_step != 0:
                    if batch_idx <= self.trainer.lightning_module.model.latest_step:
                        if batch_idx == self.trainer.lightning_module.model.latest_step:
                            latest_scenario_tokens = scenario_tokens
                        if batch_idx % (self.trainer.lightning_module.model.latest_step // 10) == 0:
                            print(f'skip: batch_idx {batch_idx} <= latest_step {self.trainer.lightning_module.model.latest_step}')
                        continue
                    else:
                        if skip:
                            if any([scenario_token == latest_scenario_token
                                    for scenario_token, latest_scenario_token in zip(scenario_tokens, latest_scenario_tokens)]):
                                continue
                            else:
                                skip = False
            ...
            # # TODO debug only
            # if scenario_tokens[0] != '5560b6be286c50d7':
            #     batch = None
            #     continue

            # # TODO debug only: manually save model
            # self.trainer.save_checkpoint(self.trainer.checkpoint_callback.dirpath + '/ritp_planner_inital_checkpoint.ckpt')

            # # TODO debug only: skip rest iterations
            # if self.env.iteration[0] > 2:
            #     self.env.episode_end = [True for episode_end in self.env.episode_end]

            self.beginning_of_scenario = [
                scenario_token != previous_scenario_token
                for scenario_token, previous_scenario_token in zip(scenario_tokens, previous_scenario_tokens)
            ]
            self.trainer.lightning_module.beginning_of_scenario = self.beginning_of_scenario

            # skip the rest iterations when done or episode_end is True and reset iteration when entering next scenario
            if all(self.env.done) or all(self.env.episode_end):
                if all([scenario_token == previous_scenario_token
                           for scenario_token, previous_scenario_token in zip(scenario_tokens, previous_scenario_tokens)]):
                    batch = None
                else:
                    self.env.iteration = [0] * num_rl_workers
                    self.env.done = [False] * num_rl_workers
                    self.env.episode_end = [False] * num_rl_workers
                    self.env.log(f'batch_idx: {batch_idx}-scenarios_tokens: {scenario_tokens}')
            previous_scenario_tokens = scenario_tokens

            # set up attributes in self.env
            if batch is not None:
                if self.env.iteration[0] == 0:  # iteration across worker_ids are consistent with each other
                    self.env.on_scenario_start(scenarios)
            ...
            with self.trainer.profiler.profile("run_training_batch"):
                ...
                #     print(e)
                    ...
                    traceback.print_exc()
                    # save checkpoint and replay_buffer before unexpected exit
                    ckpt_name = f'epoch={self.trainer.current_epoch}-step={self.trainer.global_step}-rl_step={rl_step}.ckpt'
                    ckpt_save_dir = os.path.join(self.replay_buffer.save_dir, ckpt_name)
                    self.trainer.save_checkpoint(ckpt_save_dir)
                    self.env.log(f'checkpoint saved: {ckpt_save_dir}')
                    # replay_buffer_name = f'replay_buffer-epoch={self.trainer.current_epoch}-step={self.trainer.global_step}-rl_step={rl_step}.pth'
                    # replay_save_dir = os.path.join(self.replay_buffer.save_dir, replay_buffer_name)
                    # self.replay_buffer.save(replay_save_dir)
                    # batch_output = self.run_training_batch(batch, batch_idx, dataloader_idx)
                    raise RuntimeError('run_training_batch failed')

            if batch is not None:
                if self.env.previous_state is not None:
                    add_mask = [not previous_done for previous_done in self.env.previous_done]
                    self.replay_buffer.add(state=self.env.previous_state,
                                           action=self.env.previous_action,
                                           action_samples=self.env.previous_action_samples,
                                           target=self.env.previous_target,
                                           reward=self.env.previous_reward,
                                           next_state=self.env.state,
                                           done=self.env.done,
                                           next_target=self.env.target,
                                           add_mask=add_mask)

                    # # TODO debug only
                    # while self.replay_buffer.size < 2000:
                    #     self.replay_buffer.add(state=self.env.previous_state,
                    #                            action=self.env.previous_action,
                    #                            action_samples=self.env.previous_action_samples,
                    #                            target=self.env.previous_target,
                    ...
                    #                            next_target=self.env.target,
                    #                            add_mask=add_mask)
                    # empty_imagined_state = [i for i in range(len(self.replay_buffer.next_state[:self.replay_buffer.size])) if
                    #              len(self.replay_buffer.next_state[i]['imagined_state']['vector_set_map']) == 0]
                    # if len(empty_imagined_state) > 0:
                    #     print(f'empty_imagined_state: {empty_imagined_state}')
                    #     # raise RuntimeError(f'empty_imagined_state: {empty_imagined_state}')
                self.env.iteration = [iteration + 1 for iteration in self.env.iteration]

            rl_step = self.trainer.lightning_module.model.t
            start_timesteps = self.trainer.lightning_module.model.start_timesteps
            # if is_last_batch or (is_last_scenario and (all(self.env.done) or all(self.env.episode_end))):
            # if is_last_batch or (self.trainer.global_step % 1000 == 0 and self.trainer.global_step != 0):
            # if is_last_batch or (rl_step % start_timesteps == 0 or rl_step % (num_rl_workers * 500) == 0):
            if is_last_batch or (rl_step % start_timesteps == 0):
                if rl_step != previous_rl_step:
                    ckpt_name = f'epoch={self.trainer.current_epoch}-step={self.trainer.global_step}-rl_step={rl_step}.ckpt'
                    ckpt_save_dir = os.path.join(self.replay_buffer.save_dir, ckpt_name)
                    self.trainer.save_checkpoint(ckpt_save_dir)
                    self.env.log(f'checkpoint saved: {ckpt_save_dir}')
                    previous_rl_step = rl_step
            # if is_last_batch or (rl_step % self.replay_buffer.max_size == 0 or rl_step == start_timesteps):
            #     replay_buffer_name = f'replay_buffer-epoch={self.trainer.current_epoch}-step={self.trainer.global_step}-rl_step={rl_step}.pth'
            #     replay_save_dir = os.path.join(self.replay_buffer.save_dir, replay_buffer_name)
            #     self.replay_buffer.save(replay_save_dir)
    ...
    def build_train_args(self, batch, batch_idx, opt_idx, hiddens):
        ...
        if not self.beginning_of_scenario or (self.beginning_of_scenario and opt_idx > 0):
            features, targets, scenarios = batch
            for feature in features.values():
                feature.enable_to_device = False
            for target in targets.values():
                target.enable_to_device = False
            batch = [features, targets, scenarios]
