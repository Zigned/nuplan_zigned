...
class TrainerDataLoadingMixin(ABC):
    ...
    def _get_distributed_sampler(
        ...
        if self.accelerator_connector.distributed_backend == 'ddp_sharded':
            # Zigned: make sure dataloader for GPUs are the same
            self.distributed_sampler_kwargs['num_replicas'] = 1
            self.distributed_sampler_kwargs['rank'] = 0
            kwargs = {
                'num_replicas': 1,
                'rank': 0,
            }
    ...
    def reset_train_dataloader(self, model: LightningModule) -> None:
        ...
        # automatically add samplers # modified by Zigned 202402271228
        # self.train_dataloader = apply_to_collection(
        #     self.train_dataloader, DataLoader, self.auto_add_sampler, shuffle=True
        # )
        self.train_dataloader = apply_to_collection(
            self.train_dataloader, DataLoader, self.auto_add_sampler, shuffle=False
        )
