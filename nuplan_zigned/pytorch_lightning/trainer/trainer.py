...
from nuplan_zigned.pytorch_lightning.trainer.training_loop import TrainLoop
from nuplan_zigned.pytorch_lightning.trainer.data_loading import TrainerDataLoadingMixin
...
class Trainer(
    ...
    def __init__(
        ...
        Args:
            ...
            replay_buffer: Replay buffer for RL.
            env: Environment for RL.
        ...
        self.train_loop = TrainLoop(self, multiple_trainloader_mode, replay_buffer, env)
    ...
    def run_evaluation(self, on_epoch: bool = False) -> _EVALUATE_OUTPUT:
        ...
        # variables for RL
        num_rl_workers = self.lightning_module.model.num_rl_workers
        previous_scenario_tokens = [None] * num_rl_workers
        self.train_loop.env.iteration = [0] * num_rl_workers  # timestep
        ...
        for dataloader_idx, dataloader in enumerate(dataloaders):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                self.train_loop.beginning_of_scenario = [
                ...
                self.lightning_module.beginning_of_scenario = self.train_loop.beginning_of_scenario

                # skip the rest iterations when done or episode_end is True and reset iteration when entering next scenario
                if np.all(self.train_loop.env.done) or np.all(self.train_loop.env.episode_end):
                    if np.all([scenario_token == previous_scenario_token
                               for scenario_token, previous_scenario_token in zip(scenario_tokens, previous_scenario_tokens)]):
                        batch = None
                    else:
                        self.train_loop.env.iteration = [0] * num_rl_workers
                        self.train_loop.env.done = [False] * num_rl_workers
                        self.train_loop.env.episode_end = [False] * num_rl_workers
                previous_scenario_tokens = scenario_tokens

                # generate features and targets at iteration
                if batch is not None:
                    if self.train_loop.env.iteration[0] == 0:  # iteration across worker_ids are consistent with each other
                        self.train_loop.env.on_scenario_start(scenarios)
                ...
                self.train_loop.env.iteration = [iteration + 1 for iteration in self.train_loop.env.iteration]
