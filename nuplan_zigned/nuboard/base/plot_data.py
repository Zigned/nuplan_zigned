...
class EgoStateTrajectoryPlot(BaseScenarioPlot):
    ...
    def update_plot(self, main_figure: Figure, frame_index: int, doc: Document, ego_trajectory_plotted: str, num_modes_plotted: int) -> None:
        """
        Update the plot.
        :param main_figure: The plotting figure.
        :param frame_index: Frame index.
        ...
        with self.data_source_condition:
            ...
            if ego_trajectory_plotted == 'predicted_trajectory':
                # this is the planned trajectory at frame_index
                data_sources = dict(self.data_sources[frame_index].data)
            elif ego_trajectory_plotted == 'simulated_trajectory':
                # TODO this is the resulting trajectory after simulation
                xs, ys = [], []
                for data_source in self.data_sources.values():
                    xs.append(data_source.data['xs'][0])
                    ys.append(data_source.data['ys'][0])
                data_sources = {
                    'xs': xs,
                    'ys': ys,
                }
            else:
                raise RuntimeError('ego_trajectory_plotted is neither predicted nor simulated trajectory')

            def update_main_figure() -> None:
                """Wrapper for the main_figure update logic to support multi-threading."""
                if self.plot is None:
                    if 'rest_modes' not in data_sources.keys():
                        ...
                        xs, ys = [], []
                        for i, mode in enumerate(data_sources['rest_modes']):
                            xs.append(mode['xs'])
                            ys.append(mode['ys'])
                            if i >= num_modes_plotted - 1:
                                break
                        data_sources_rest_modes = {
                            'xs': xs,
                            'ys': ys,
                        }
                        self.plot = main_figure.multi_line(
                            ...
                            ys="ys",
                            line_color=simulation_tile_trajectory_style["ego"]["line_color"],
                            line_width=simulation_tile_trajectory_style["ego"]["line_width"] / 2,
                            line_alpha=simulation_tile_trajectory_style["ego"]["line_alpha"],
                            source=data_sources_rest_modes,
                        )
                else:
                    self.plot.data_source.data = data_sources
    ...
    def update_data_sources(self, history: SimulationHistory) -> None:
        ...
        with self.data_source_condition:
            for frame_index, sample in enumerate(history.data):
                ...
                if hasattr(sample, 'rest_modes'):
                    if sample.rest_modes is not None:
                        rest_modes = [traj.get_sampled_trajectory() for traj in sample.rest_modes]  # TODO used for plotting the rest modes
                        rest_coords = []
                        for traj in rest_modes:
                            x_coords_rest_modes = []
                            y_coords_rest_modes = []
                            for state in traj:
                                x_coords_rest_modes.append(state.center.x)
                                y_coords_rest_modes.append(state.center.y)
                            rest_coords.append(
                                {'xs': x_coords_rest_modes, 'ys': y_coords_rest_modes}
                            )
                        source = ColumnDataSource(dict(xs=x_coords, ys=y_coords, rest_modes=rest_coords))
                self.data_sources[frame_index] = source
                self.data_source_condition.notify()
