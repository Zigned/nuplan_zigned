...
from nuplan_zigned.torch_geometric.utils.num_nodes import maybe_num_nodes
...
def coalesce(
    ...
    if isinstance(edge_attr, Tensor):
        ...
        # reduce == 'max'
        # print('performing extremely expensive scatter_max in line 142 of coalesce!')
        edge_attr = scatter_max(edge_attr, idx, 0, dim_size)
        return edge_index, edge_attr
...
def scatter_max(src, index, dim=0, dim_size=None):
    """
    Manual implementation of scatter_max. Scatters `src` into the `out` tensor based on `index`.
    Keeps the max value at each index position.

    Args:
        src (Tensor): The source tensor to be scattered.
        index (LongTensor): The index tensor specifying where to scatter.
        dim (int): The dimension along which to scatter (default is 0).
        dim_size (int, optional): The size of the dimension to scatter to (default is None, will be inferred).

    Returns:
        Tensor: The output tensor containing the maximum values.
    """
    # Create an empty tensor to store the results
    out = torch.full((dim_size,), 0, dtype=src.dtype, device=src.device)

    # Scatter the values to the target positions
    for i in range(src.size(0)):
        idx = index[i]
        out[idx] = torch.max(out[idx], src[i])  # Keep the max value
