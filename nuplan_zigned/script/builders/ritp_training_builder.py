...
from nuplan_zigned.training.modeling.ritp_lightning_module_wrapper import LightningModuleWrapper
from nuplan_zigned.training.data_loader.ritp_datamodule import DataModule
from nuplan_zigned.pytorch_lightning.trainer.trainer import Trainer
...
def build_lightning_datamodule(
    ...
    # Create data augmentation (moved to build_lightning_module)
    augmentors = None
    ...
    datamodule: pl.LightningDataModule = DataModule(
        ...
        num_rl_workers=cfg.model.ritp_planner_model_params.num_rl_workers
        if 'ritp_planner_model_params' in cfg.model
        else cfg.model.single_step_planner_model_params.num_rl_workers,
...
def build_trainer(cfg: DictConfig) -> pl.Trainer:
    ...
    if 'resume_from_checkpoint' in params.keys():
        logger.info(f'Resuming from checkpoint {params.resume_from_checkpoint}')
    ...
    trainer = Trainer(
        ...
        replay_buffer=ReplayBuffer(max_size=int(cfg.utils.replay_buffer.max_size),
                                   ...
                                   load_dir=cfg.utils.replay_buffer.load_dir,
                                   save_dir=cfg.output_dir),
        env=Env(v_min=cfg.utils.env.v_min,
                v_max=cfg.utils.env.v_max,
                a_min=cfg.utils.env.a_min,
                a_max=cfg.utils.env.a_max,
                lane_width=cfg.utils.env.lane_width,
                mu=cfg.utils.env.mu,
                g=cfg.utils.env.g,
                T=cfg.utils.env.T,
                num_poses=cfg.utils.env.num_poses,
                ego_controller_config=cfg.utils.env.ego_controller,
                num_rl_workers=cfg.utils.env.num_rl_workers),
        **params,
    )
