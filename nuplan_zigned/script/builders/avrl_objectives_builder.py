...
def build_objectives(cfg: DictConfig, reward: TorchModuleWrapper) -> List[AbstractObjective]:
    """
    ...
    for objective_name, objective_type in cfg.objective.items():
        new_objective: AbstractObjective = instantiate(
            objective_type,
            reward=reward,
            scenario_type_loss_weighting=scenario_type_loss_weighting,
            base=objective_type.base
        )
