...
from nuplan.common.utils.distributed_scenario_filter import DistributedMode
...
from nuplan_zigned.script.builders.planner_builder import build_planners
from nuplan_zigned.utils.batched_distributed_scenario_filter import DistributedScenarioFilter
from nuplan_zigned.simulation.runner.simulations_runner import SimulationRunner
from nuplan_zigned.simulation.simulation import Simulation
...
def build_simulations(
    ...
    # only simulate the scenarios in the validation set or testing set
    splitter = build_splitter(cfg.splitter)
    if cfg.val_test_scenarios == 'val':
        # only simulate the scenarios in the validation set
        val_samples = splitter.get_val_samples(scenarios, worker)
        assert len(val_samples) > 0, 'Splitter returned no val samples'
        scenarios = val_samples
        logger.info(f"Number of samples in validation set: {len(scenarios)}")
    else:
        # only simulate the scenarios in the test set
        test_samples = splitter.get_test_samples(scenarios, worker)
        assert len(test_samples) > 0, 'Splitter returned no test samples'
        scenarios = test_samples
        logger.info(f"Number of samples in testing set: {len(scenarios)}")
