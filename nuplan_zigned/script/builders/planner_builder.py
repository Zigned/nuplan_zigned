...
from nuplan_zigned.simulation.planner.irl_planner import IRLPlanner
from nuplan_zigned.simulation.planner.ritp_planner import RITPPlanner
from nuplan_zigned.simulation.planner.qcmae_planner import QCMAEPlanner
...
def _build_planner(planner_cfg: DictConfig, scenario: Optional[AbstractScenario]) -> AbstractPlanner:
    ...
    logger.info(f'Planner checkpoint path: {planner_cfg.checkpoint_path}')
    if is_target_type(planner_cfg, MLPlanner):
    ...
    elif is_target_type(planner_cfg, IRLPlanner):
        # Build model and feature builders needed to run an ML model in simulation
        torch_module_wrapper = build_torch_module_wrapper(planner_cfg.model_config)
        model = LightningModuleWrapper.load_from_checkpoint(
            planner_cfg.checkpoint_path, model=torch_module_wrapper
    ...
    elif is_target_type(planner_cfg, RITPPlanner):
        # Build model and feature builders needed to run an ML model in simulation
        torch_module_wrapper = build_torch_module_wrapper(planner_cfg.model_config)
        model = LightningModuleWrapper.load_from_checkpoint(
            planner_cfg.checkpoint_path, model=torch_module_wrapper, strict=False
        ).model
        ...
        planner: AbstractPlanner = instantiate(config, model=model, scenario=scenario)
    elif is_target_type(planner_cfg, QCMAEPlanner):
        # Build model and feature builders needed to run an ML model in simulation
        torch_module_wrapper = build_torch_module_wrapper(planner_cfg.model_config)
        model = LightningModuleWrapper.load_from_checkpoint(
            planner_cfg.checkpoint_path, model=torch_module_wrapper, strict=False
